...
3. BiST mode: This mode revolves around using the Pseudo Random Number Generator (a.k.a. LFSR) and Singature Analyzer to do at-speed testing. It overlaps extensively with the External mode. The difference comes in the test configuration (test_config_struct) as now the input activations are generated by the LFSR, streamed into the systolic array, and then compressed on the Signature Analyzer. Once the Signature is ready (which is determined by the LFSR's stop code) it will be used to determine if incorrect values were produced. This mode helps us isolate the design from: (1) the memory interface, and (2) the external signal data ports. This setup allows to test the design at maximum speed.

   In this mode:
      1. The external signal ports are leveraged to pre-load the weights into the array (similar to External mode)
      2. Afterwards, the LFSR is enabled (test_config_struct.driver_valid) and generates pseudo random data that is fed into your design (take careful note of what ports are driven with the LSFR's outputs).
      3. After the data propagates through your design, it will reach the Signature Analyzer, which will compress valid data cycle by cycle until no more valid data is received. Your module is responsible for receiving the LSFR's valid signal and adequately sending a valid signal to the Signature Analyzer. 

   Shown below is an example of the input and expected output pattern.

<p align="center">
<img src="./img/signals_bist.png" alt="" width="700"/>
</p>

**Your design should be parameterizable. You may be asked to change the number of rows, columns, and data width before submitting the final version**

**The Matrix Mult Wrapper and Pkg have been provided**
...
## Data & Memory Format & Assumptions
Input data assumptions:

1. Data is stored on global memories (Memory Mode)
2. Comes from an external input (External Mode)
3. Generated by a pseudo random number generator (BiST Mode)

Memory Format & Mapping:

Note that K = 4, N = 4, are given parameters.

<p align="center">
<img src="./img/memory_mapping.png" alt="" width="700"/>
</p>
