...
~~The valid and data input ports are also used to set the seed of LFSR and SA when in driver mode 1 or monitor mode 1. Send the seed through data port with one cycle of valid in the beginning of your test sequence if you are using LFSR or SA. LFSR stops when the generated pattern matches the stop pattern from the stop code input port. ~~
